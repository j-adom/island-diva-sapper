...
<script>
    ...
    export let category
    category.posts.forEach(article => {
        article.url = 'blog/posts/' + article.slug.current
    });

    let featPosts = category.posts.filter(post => post.featured ==true )
    const months = [
      'January',
      'February',
      'March',
                ...
                <div class="cms-collection w-dyn-list">
                    <div role="list" class="cms-list w-dyn-items">
                        <div role="listitem" class="cms-item fade-in-1st w-dyn-item">
                            <a href={category.posts[0].url} data-w-id="7a0255d2-0be4-a968-2d7a-dd00ab3eedd8" class="image-filled-featured w-inline-block">
                                <img data-w-id="7a0255d2-0be4-a968-2d7a-dd00ab3eedd9" src={urlFor(category.posts[0].mainImage).url()} alt={category.posts[0].mainImage.alt} class="bg-image">
                                <div class="gradient-bottom"></div>
                                <div class="w-layout-grid thumb-info-small-grid">
                                    <div class="category-wrapper align-center">
                                        <div>{formatDate(category.posts[0].publishedAt)}</div>
                                        <!-- <div class="caption-dot">·</div>
                                        <div></div><div> min</div> -->
                                    </div>
                                    <div>{category.posts[0].title}</div>
                                    <div id="w-node-dd00ab3eedde-626961c2" class="author-wrapper-italic">
                                        <div>by </div>
                                        <div>{category.posts[0].authors[0].name}</div>
                                    </div>
                                    <div id="w-node-dd00ab3eede3-626961c2" data-w-id="7a0255d2-0be4-a968-2d7a-dd00ab3eede3" class="post-summary">
                                        <div>
                                            <BlockContent blocks={category.posts[0].excerpt} {serializers} />
                                        </div>
                                    </div>
                                </div>
                            </a>
